...
public class StackController : MonoBehaviour {
    ...
    private int score=0,Flag=1,j=0, top,k=10;
    private float X=0,Z=0,scx=5,scz=5,speed = 2f,tileTransition=0.0f;
    public int topIndex;
   ...
   //     i = (float)(stack[transform.childCount-2].transform.position.x);
	...
	void Update () {
        ...
        if (!gameFlag && Input.GetKeyDown("r"))
            Reset();
        if (gameFlag)
        {
    ...
    private void Reset()
    {
        ...
        for (j = 0; j < transform.childCount; j++)
        {
            stack[j].transform.localPosition = new Vector3(X, j, Z);
        }
